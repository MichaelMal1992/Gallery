...
extension GalleryViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let images = ImageDataManager.shared.get
        if let image = images.first(where: {$0.name == PictureManager.shared.currentName}) {
            cell.commentLabel.text = image.comments[indexPath.row].text
            cell.timeLabel.text = image.comments[indexPath.row].time
            cell.commentContainerView.backgroundColor = .clear
        }
        return cell
    }
}
